...
class CreateGraph:
    ...
    ########################functions for maps############################
        ...
        def compositionWithDBGCons2(read):
            ...
            return kmerList  # , inputEdgeGraph
        ...
        def sumFreq(item):
            ...
            # return ((item[0], sumfreq),outnodelist)
    ...
    ######################################################################
        ...
        if loop_count == 1:
            ...
            # # print(seqListRDD.take(10))
            # seqListRDD = seqListRDD.filter(lambda x: not x.startswith('@')).map(lambda x: x.strip('\n').split('\t'))
            # print("-> Select rows by AS < len(seq) , .rdd.map(lambda x: x[0])")
            # # print(seqListRDD.take(10))
            # seqListRDD = seqListRDD\
            #     .filter(lambda x: (x[11].split(':')[0] == "AS") & (int(x[11].split(':')[2]) < len(x[9]))
            #                       | (x[13].split(':')[0] == "AS") & (int(x[13].split(':')[2]) < len(x[9])))\
            #     .map(lambda x: x[9])
            self.time_duration(start_time, "sc.parallelize", False)

            # print("- input_dir:", bwa_output_list)
            # schema1 = StructType([StructField("c0", StringType(), True), StructField("c1", StringType(), True),
            ...
            # print("-> Reading bwa_output_file")
            # start_time = datetime.now()
            # df_tmp = self.spark.read.schema(schema1).options(sep="\t", comment="@").csv(bwa_output_list)
            # self.time_duration(start_time, "spark.read.csv(bwa_output_file)"),
            #
            # print("-> Select rows by AS < len(seq) , .rdd.map(lambda x: x[0])")
            # seqListRDD = df_tmp.select("c9").filter(
            #     ((split("c13", ":").getItem(0) == "AS") & (split("c13", ":").getItem(2) < length(col("c9")))) |
            #     ((split("c11", ":").getItem(0) == "AS") & (split("c11", ":").getItem(2) < length(col("c9"))))) \
            #     .rdd.map(lambda x: x[0])

            ### save remain reads to file for use in sparkbwa
            ### get remain reads and set id with zipWithUniqueId and save to a fasta or fastq file
            # print("-> Start zipWithUniqueId().map().saveAsTextFile(remain_reads)")
            # start_time = datetime.now()
            # seqListRDD.zipWithUniqueId().map(lambda x: f">{x[1]}\n{x[0]}").coalesce(1).saveAsTextFile(remain_reads)
            # seqListRDD.zipWithUniqueId().map(lambda x: f"@{x[1]}\n{x[0]}\n+\n" + "?" * len(x[0])).coalesce(1)\
            #     .saveAsTextFile(remain_reads)
            # self.time_duration(start_time, "zipWithUniqueId().map().saveAsTextFile(remain_reads)")


        # print("seqListRDD.count():", seqListRDD.count())
        ...
        # print("-> graphRDD3.zipWithUniqueId().map()")
        # start_time = datetime.now()
        # graphRDD3 = graphRDD3.zipWithUniqueId() \
        #     .map(lambda x: (x[1], x[0][0], x[0][1], x[0][2], x[0][3], x[0][4], x[0][5]))
        # print(f"-> {graphRDD3.getNumPartitions()=}")
        # print(f"==>> {graphRDD3.first()=}")
        # self.time_duration(start_time, "graphRDD3.zipWithUniqueId().map()", False)
        #
        # start_time = datetime.now()
        # print("-> start graphRDD3.coalesce(70)")
        # graphRDD3 = graphRDD3.coalesce(70)
        # print(f"-> {graphRDD3.getNumPartitions()=}")
        # print(f"==>> {graphRDD3.first()=}")
        # self.time_duration(start_time, "graphRDD3.coalesce(70)")
        #
        # df2 = graphRDD3.toDF(schema)

        df1 = df2.select("id", "kmer")
        print(f"-> {df1.rdd.getNumPartitions()=}")

        # print("-> df2.repartition('o1')")
        # start_time = datetime.now()
        # df2 = df2.repartition("o1")
        # print(f"-> {df2.rdd.getNumPartitions()=}")
        # print(f"==>> {df2.first()=}")
        # self.time_duration(start_time, "df2.repartition('o1')", False)
        ...
        # print("-> Separately df2.join(df1).join(df1).join(df1).join(df1).join(df1)")
        # df_dbg = df2.alias('graph').join(df1.alias('mapId1'), col("o1") == col("mapId1.kmer"), "left") \
        #     .select(col('graph.id').alias('kmer_id'), col('graph.kmer').alias('node_kmer'), 'freq',
        #             col('mapId1.id').alias('o1_id'), 'graph.o2', 'graph.o3', 'graph.o4')
        # df_dbg = df_dbg.alias('graph').join(df1.alias('mapId2'), col("o2") == col("mapId2.kmer"), "left") \
        #     .select('kmer_id', 'node_kmer', 'freq', 'o1_id', col('mapId2.id').alias('o2_id'), 'graph.o3', 'graph.o4')
        # df_dbg = df_dbg.alias('graph').join(df1.alias('mapId3'), col("o3") == col("mapId3.kmer"), "left") \
        #     .select('kmer_id', 'node_kmer', 'freq', 'o1_id', 'o2_id', col('mapId3.id').alias('o3_id'), 'graph.o4')
        # df_dbg = df_dbg.alias('graph').join(df1.alias('mapId4'), col("o4") == col("mapId4.kmer"), "left") \
        #     .select('kmer_id', 'node_kmer', 'freq', 'o1_id', 'o2_id', 'o3_id', col('mapId4.id').alias('o4_id'))
        ...
        # print("-> df_dbg.write.csv")
        df_dbg.coalesce(1).write.mode("append").csv(self.out_dir_graph)#+"/df_dbg")
        # df_dbg.write.mode("append").csv(self.out_dir_graph)#+"/df_dbg")
        self.time_duration(start_time, "df_dbg.coalesce(1).write.csv", True)
        # self.time_duration(start_time, "df_dbg.write.csv", True)
    ...
    def create_string_graph(self, df_dbg):
        ...
        # rdd1 = self.sc.textFile(self.input_contigs).zipWithIndex().filter(lambda x: x[1] & 3 == 1).map(lambda x: x[0])
        # print("Contigs count:", rdd1.count())
        ...
        # df_contigs = rdd1.flatMap(lambda r: (r, str(Seq(r).reverse_complement()))).map(lambda x: (x,)).toDF(["contig"])
        # print("Contigs+RC count:", df_contigs.count())
        ...
        # df_pref = df_pref.select(df_pref.contig_id.alias("cng_id_pref"), "kmer_id") \
        #     .groupBy("cng_id_pref").agg(F.collect_list("kmer_id").alias("out_list"))
...
def correction_with_karect():
    karect_output_dir = "/home/myuser/SparkProjects/DBGConstructionwithSpark/KarectedFiles"
    karect_cmd = f"/home/myuser/SparkProjects/DBGConstructionwithSpark/karect -correct -inputfile={input_file1} -inputfile={input_file2} " \
                 f"-resultdir={karect_output_dir} -matchtype=hamming -celltype=haploid"

    cwd = os.getcwd()
    print(cwd)
    # os.makedirs("./KarectedFiles", exist_ok=True)
    if not os.path.exists(karect_output):
...
def start_spark(conf):
    ...
    #     [('spark.app.name', 'DBGConst_v5_addStringGraph.py'), ("Master", "spark://master:7077"),
    #      ("spark.deployMode", "client")])

    # conf = SparkConf().getAll()

    # conf = SparkConf().setAll(
    #     [("Master", "local[*]"), ("spark.executor.memory", "4g"), ('spark.driver.memory', '4g'),
...
def main(inp_file1: str, inp_file2: str):
    ...
    for k in k_list:
        ...
        if loop_count == 1:
            # spark_config = [('spark.master', 'spark://myipmdebian:7077'), ('spark.app.name', 'Distributed_RMI_v2_centralSamAnalysis_000328.py'), ('spark.cores.max', '20')]
            # spark_config = SparkConf().getAll()
            # spark_config.set('spark.cores.max', '20')
            spark_config = SparkConf()
            # spark_config.set("spark.sql.shuffle.partitions", "400")
        else:
            spark_config = SparkConf()
            # spark_config.set("spark.sql.shuffle.partitions", "400")
            # spark_config.setMaster("spark://myipmdefbian:7077")
            # spark_config.set('spark.cores.max', '16')
        ...
        # cmd = f"giraph ~/eclipse-workspace/DBG-Giraph_17.jar "
        ...
        if workers_num == 1:
            ...
            # f"-w 1 -ca giraph.SplitMasterWorker=false -ca k_value={k_value - 1}"
        ...
        # cmd = f"~/PycharmProjects/Tools/bbmap/dedupe.sh in={bbmap_input} out={bbmap_output}"
        ...
        if loop_count < max_loop_count:
            ...
            # cmd = f"hdfs dfs -put {bwa_output_on_fs} {output_sub_dir}"
            # run_cmd(cmd, "hdfs dfs -put bwa_output")
            # bwa_output_on_hdfs = f"{output_sub_dir}/{bwa_output_file}"

        else:
            log_str += "\t\t\t\t"

        input_contigs = bbmap_output
        ### for reading bbmap output from master disk(fs)

        # input_contigs = f"{output_sub_dir}/{os.path.basename(bbmap_fq_output)}"

        # remain_reads = output_sub_dir + "/remain_reads"
...
def sga_quast(file1, file2, data_dir, refGenFileAddress, in_f1, in_f2):
    ...
    # strCommand = str_precommand + " sga overlap -m " + str(k-11) + " -t 4 " + cngwithrc_name
...
if __name__ == '__main__':
    ...
    # dsname = os.path.basename(ref_file).split(".")[0].split("_")[0]
    # dsname = os.path.basename(ref_file).split(".")[0]
